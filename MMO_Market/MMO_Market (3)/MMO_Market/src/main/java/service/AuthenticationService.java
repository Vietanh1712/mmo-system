...
public class AuthenticationService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (Boolean.TRUE.equals(user.getIsLocked())) {
            log.warn("Tai khoan dang bi khoa: {}", request.getEmail());
            return LoginResponse.builder().message("Tài khoản đã bị khóa. Vui lòng liên hệ bộ phận hỗ trợ.").build();
        }
    ...
    public LoginResponse refreshAccessToken(String refreshToken) {
        ...
        if (Boolean.TRUE.equals(user.getIsLocked())) {
            return LoginResponse.builder().message("Tài khoản đã bị khóa. Vui lòng đăng nhập lại sau khi được mở khóa.").build();
        }
