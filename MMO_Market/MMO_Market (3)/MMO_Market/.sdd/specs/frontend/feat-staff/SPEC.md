...
| FR-STAF-30 | WHEN Staff views the `/staff/shop-registrations` page, THE SYSTEM SHALL query the database and display shop registration statistics (total, pending, approved, and rejected). |
| FR-STAF-31 | WHEN Staff searches or filters shop registrations on `/staff/shop-registrations`, THE SYSTEM SHALL query the database using the keyword, request status (`status`), and shop account status (`shopStatus`). |
...
| FR-STAF-37 | THE SYSTEM SHALL dynamically load the registration request status filter (`status`) dropdown from the `GET /api/v1/shop-registrations/statuses` endpoint to fetch distinct registration statuses from the database. |
