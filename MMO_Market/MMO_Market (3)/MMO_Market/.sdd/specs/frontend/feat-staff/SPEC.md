...
| FR-STAF-34 | WHEN displaying the shop account status (`shopStatus`) badge on `/staff/shop-registrations`, THE SYSTEM SHALL translate database values to standardized Vietnamese: Active ("Hoạt động"), Suspended ("Tạm ngưng"), and Withdrawn ("Đã đóng Shop (Hoàn cọc)"). Stat cards SHALL display: "Shop tạm ngưng" and "Đã đóng Shop (Hoàn cọc)". |
...
| FR-STAF-55 | WHEN Staff selects the "Tạm ngưng" (Suspended) status for a shop, THE SYSTEM SHALL launch a modal dialog (`suspendShopModal`) requiring Staff to specify the expiration Date, Hour, and Minute (`suspendedUntil`) before submitting. |
| FR-STAF-56 | WHEN a shop is in Suspended status, THE SYSTEM SHALL display a highlighted alert card (`cardSuspendedAlert`) inside the "Cập nhật trạng thái hoạt động" side card containing the exact deadline date and a real-time countdown timer ("Tự động mở lại sau"). |
...
| FR-STAF-60 | WHEN a shop's operating status is already "Withdrawn" or "Deleted" (permanently closed and refunded), THE SYSTEM SHALL hide the status update form/section and display a permanent alert indicating that the shop is permanently closed and refunded. |
| FR-STAF-61 | THE SYSTEM SHALL restrict Shop Flag warning levels to exactly 3 options: Cảnh cáo (Warning - Level 1), Nguy hiểm (Danger - Level 2), and Cấm (Ban - Level 3). Level 2 flags (Danger / Suspension) shall trigger a temporary shop suspension. |
| FR-STAF-62 | WHEN a Seller is assigned 3 or more active violation flags, THE SYSTEM SHALL automatically change their shop status to Banned ("Khóa vĩnh viễn"), clear any temporary suspension timer, and block access to the Seller Console (Seller Dashboard). |
| FR-STAF-63 | THE SYSTEM SHALL display the email of the Seller in the "Seller" column instead of the staff's email in the Shop Flags table on `/staff/flags`. |