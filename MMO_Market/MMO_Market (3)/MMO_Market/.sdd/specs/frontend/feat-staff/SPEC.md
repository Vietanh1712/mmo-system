...
| FR-STAF-45 | THE SYSTEM SHALL query and display the count of "Giao dịch chờ duyệt" and "Nạp tiền cần xử lý" alongside open complaints, pending withdrawals, and pending support tickets on the documents overview page (`/staff/documents`). |
...
| FR-STAF-58 | WHEN Staff accesses the documents overview page (`/staff/documents`), THE SYSTEM SHALL render a dedicated "Nạp tiền cần xử lý" statistic card displaying the total count of failed and pending top-up transactions (`dashboard.pendingTopups`) retrieved from the database. |