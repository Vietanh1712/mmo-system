...
> **Tham chiếu FR:** FR-WALL-01 đến FR-WALL-05
...
- Người dùng đã đăng nhập hệ thống (không phải Staff/Admin).
...
- **Thành công:** Tiền trong ví của User (`balanceVnd`) tăng thêm bằng số tiền nạp. Hệ thống lưu thành công bản ghi ở bảng `TopupTransactions` (trạng thái `Success`) và sổ cái ví `WalletTransactions` (loại `TOPUP`).
- **Thất bại:** Lưu thông tin giao dịch lỗi cú pháp/hạn mức vào bảng `TopupTransactions` (trạng thái `Failed` kèm lý do lỗi `failureReason`).
...
Bước 6  [Backend]:    POST /api/sepay/webhook (kèm Header Authorization "Apikey <sepayWebhookToken>" để xác thực).
...
Bước 8  [Backend]:    `TopupService` kiểm tra:
                       ...
                       - Mã `sepayCode` (ID của webhook) chưa tồn tại trong bảng `TopupTransactions`.
...
Bước 12 [Backend]:    Lưu thông tin giao dịch vào `TopupTransactions` và `WalletTransactions`.
...
Bước 8  [Backend]:    Tra cứu mã `sepayCode` (ID) trong bảng `TopupTransactions`. Phát hiện đã tồn tại (do Webhook bị retry hoặc gọi trùng).
...
| BR-07-03 | Nạp tiền sai cú pháp | Nếu Webhook không khớp Regex, hệ thống trả về HTTP 400. Lưu giao dịch lỗi cú pháp/hạn mức vào `TopupTransactions` với trạng thái `Failed`. Staff/Admin có thể duyệt nạp thủ công giao dịch lỗi này qua API `/api/v1/staff/topups/{id}/retry`. |
...
sequenceDiagram
    ...
    TC-->>FE: HTTP 200 {bankId, accountNumber, accountName}
    Note over FE: User quét mã tự sinh và thanh toán
    SC->>TC: POST /api/sepay/webhook {content, transferAmount, id, transferType: 'in'}
    ...
    alt Đã tồn tại
        ...
        TS->>DB: INSERT TopupTransactions (Success)
        TS->>DB: INSERT WalletTransactions (TOPUP)
        TS-->>TC: true
        TC-->>SC: HTTP 200 OK
    end
```
...
| `POST` | `/api/v1/staff/topups/{id}/retry` | Staff duyệt nạp thủ công lệnh nạp tiền bị lỗi cú pháp |
