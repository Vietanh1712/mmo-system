...
> **Lưu ý:** Các API liên quan đến quản lý sản phẩm, biến thể, và tài sản số dành cho Seller (như `POST /api/seller/products`, `POST /api/seller/variants`, `POST /api/seller/digital-assets`) được cài đặt tại `SellerController` và đã được đặc tả chi tiết trong tài liệu của module `feat-seller` (`feat-seller/SPEC.md`). Vui lòng tham khảo tài liệu đó để biết chi tiết Request/Response.