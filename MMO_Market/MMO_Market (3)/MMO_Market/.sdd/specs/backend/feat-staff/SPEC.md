...
| **FR-STF-18** | WHEN a Staff updates a shop status to 'Suspended' or 'Locked' with an expiration timestamp (`suspendedUntil`), THE SYSTEM SHALL save `suspended_until` in the Users table and automatically revert the shop status to 'Active' once the timestamp expires via scheduled job or read-trigger. |
...
| **FR-STF-23** | WHEN a Seller whose shop is in 'Locked' or 'Suspended' status calls any `/api/seller/**` endpoint, THE SYSTEM SHALL throw an `IllegalStateException` formatted as `SHOP_TEMPORARILY_LOCKED:{untilDate}` and prevent access until `suspendedUntil` expires. |
| **FR-STF-24** | WHEN `suspendedUntil` passes, THE SYSTEM SHALL automatically clear `suspendedUntil`, revert `shopStatus` to `Active`, and allow full API and dashboard access for the seller. |
