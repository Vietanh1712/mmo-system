...
| **FR-SELL-17** | WHEN a Seller queries `GET /api/v1/seller/shop-info` or `GET /api/seller/dashboard`, THE SYSTEM SHALL check `suspendedUntil`. IF `suspendedUntil` has expired, IT SHALL automatically reset `shopStatus` to `Active` and save user; IF `suspendedUntil` is in the future or unexpired for a locked/suspended shop, IT SHALL reject the request with HTTP 400 Bad Request and error message `SHOP_TEMPORARILY_LOCKED:{untilDate}` to block seller portal access. |
