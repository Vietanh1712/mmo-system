...
- **FR-STAF-08**: WHEN Staff manages shop account statuses, THE SYSTEM SHALL support 3 distinct status options with Vietnamese labels: Active ("Hoạt động"), Suspended ("Tạm ngưng"), and Withdrawn ("Đã đóng Shop (Hoàn cọc)").
...
- **FR-STAF-12**: WHEN a shop's status is already "Withdrawn" or "Deleted" (permanently closed and refunded), THE SYSTEM SHALL hide the status update controls and display a warning notice to prevent further modifications.
- **FR-STAF-13**: WHEN a Seller is assigned 3 or more active violation flags, THE SYSTEM SHALL automatically change the shop's status to Banned ("Khóa vĩnh viễn"), clear any temporary suspension timer, and block access to the Seller Console (Seller Dashboard).
