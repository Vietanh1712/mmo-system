// =======================================================
// LOGIC TRANG DANH MỤC SẢN PHẨM (CATALOG PAGE)
// =======================================================
...
// Biến trạng thái toàn cục của trang danh mục
let currentKeyword = '';
...
// Xác định danh mục cha từ tên sản phẩm (dự phòng cho mock data)
...
// Lấy nhãn tiêu đề danh mục hiển thị trên trang
function getCatalogDisplayLabel() {
    if (currentKeyword) return `Tìm kiếm "${currentKeyword}"`;
    if (currentSubCategory) return currentSubCategory;
    if (currentCategory) return CATEGORY_NAMES[currentCategory] || 'Tất cả';
...
// Đồng bộ trạng thái bộ lọc giao diện theo tham số URL hiện tại
function syncUIFromURL() {
    ...
    // Hiển thị từ khóa đang tìm kiếm vào ô tìm kiếm của header
    ...
    const displayLabel = getCatalogDisplayLabel();
...
function loadInteractiveMock() {
    ...
    // 2. Lọc theo danh mục cha (bao gồm "Khác" = 7)
    if (currentCategory) {
...
// Cập nhật số liệu thống kê trên tiêu đề trang
function updateResultSummary(total, start, end) {
    ...
    if (descText) {
        descText.innerHTML = total > 0
            ? `Chúng tôi có <strong>${total}</strong> sản phẩm số chất lượng phù hợp với bộ lọc của bạn.`
            : 'Khám phá các sản phẩm số chất lượng, uy tín hàng đầu trên thị trường.';
    }
}
...
async function initCategoryFilterDropdown() {
    ...
    try {
        ...
        // Đẩy danh mục "Khác" và "Dịch vụ khác" xuống cuối
        categories.sort((a, b) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Lắng nghe thay đổi bộ lọc để tự động cập nhật kết quả
    const filterForm = document.getElementById('filter-form');
    if (filterForm) {
        filterForm.querySelectorAll('select, input[type="checkbox"]').forEach(el => {
            el.addEventListener('change', () => applyFilters());
        });
        // Lắng nghe khi người dùng thả thanh trượt giá
...
// Xử lý nút "Mua ngay" — chuyển đến trang chi tiết sản phẩm
...
// Xử lý nút "Đặt trước" — chuyển đến trang chi tiết sản phẩm hết hàng
...
// Xử lý thêm sản phẩm vào giỏ hàng từ card trong danh mục
function addToCartFromCard(productId) {
    if (typeof window.isEmployeeUser === 'function' && window.isEmployeeUser()) {
        return; // Tài khoản nhân viên không được thêm vào giỏ
    }
